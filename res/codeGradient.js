...
function stepEleven(dataForGradientAlg, algResult) {
    ...
    let resPoint = math.matrix([['-t'], ['t']]);

    console.log('11st = ' + resPoint);
    dataForGradientAlg.resPoint = resPoint;
...
function stepTwelwe(dataForGradientAlg, algResult) {
    ...
    //console.log(math.parse(dataForGradientAlg.Fx, scope).toString());
    //const f = math.eval(`f(t) = ` + dataForGradientAlg.Fx, scope);
    //const T = math.eval(`f(t) = ` + dataForGradientAlg.Fx);


    for (let tk = 0; tk <= 1; tk += 0.01) {

        let hscope = {
            t: tk
        }
        let scope = {
            [dataForGradientAlg.variablesFx[0]]: math.eval(math.subset(dataForGradientAlg.resPoint, math.index(0, 0)), hscope),
            [dataForGradientAlg.variablesFx[1]]: math.eval(math.subset(dataForGradientAlg.resPoint, math.index(1, 0)), hscope)
        }
        let currentResult = math.eval(dataForGradientAlg.Fx, scope);
        ;

        if (currentResult < minF) {
            minF = currentResult;
            resultTk = tk;
        }
    }
    console.log(resultTk);
    dataForGradientAlg.resultTk = resultTk;
    setPrettyTr("Шаг 12", `t* = ${resultTk}`, '');
}

//дета ашыбка, мб как и в 11 шаге
function stepThreaten(dataForGradientAlg, algResult) {

    let scope = {
        t: dataForGradientAlg.resultTk,
        xk: dataForGradientAlg.xk,
        deltaXk: dataForGradientAlg.deltaXk,
        sigma2Xk: dataForGradientAlg.sigma2Xk
    }


    let plusXk = math.eval('xk + t * deltaXk + sigma2Xk', scope);
    dataForGradientAlg.xk = plusXk;
    console.log(dataForGradientAlg.xk);
    dataForGradientAlg.numderOfIteration++;
}
...
function mainRoot(dataForGradientAlg, algResult) {
    ...
    stepEleven(dataForGradientAlg, algResult);
    stepTwelwe(dataForGradientAlg, algResult);
    stepThreaten(dataForGradientAlg, algResult);
}
